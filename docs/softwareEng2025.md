...
# コーディング
コードは、書くよりも読まれることが多いので、読みやすいコードを書くことは必須である。PythonのPEP8において、コードの1行の長さは79文字以内にして、docstringやコメントの長さは72文字以内にするという決まりがある。次の行で前の行を継続する場合は、折り返された要素を縦に揃える。加えて、コードのレイアウトは、1レベルインデントするごとに、スペースを4つ使わなければならない。そして、演算子の前後にはスペースを前後に一つずつ空け、その他の部分では無駄なスペースを入れないというルールがある。また、改行する時に、文を重ねない、演算子の位置を揃えるというルールもある。
# バージョン管理
バージョン管理は、ファイルを「誰が」、「いつ」、「どのように」変更したのかを管理する。これらの情報が管理されていれば、多数のメンバーとも共同でコード開発ができる。バージョン管理の方法には、集中管理型と分散管理型がある。集中管理型とは、リモートレポジトリでのみバージョン管理を行う方法である。これには、同時編集するとコンフリクトが発生しやすい、同期に時間がかかるという特徴がある。分散管理型とは、リモートレポジトリだけでなくローカルレポジトリでもバージョン管理を行う方法である。これには、リモートレポジトリへのアクセス頻度が低い、障害に頑健という特徴がある。
# git
gitとは、分散管理型のバージョン管理システムである。gitには、変更履歴が残る、変更した箇所に戻ることができる、他人と共同編集できるというメリットがある。gitの機能には、コミット、レポジトリがある。コミットは、ファイルの作成、変更、削除の記録である。レポジトリは、gitが管理するプロジェクトのフォルダである。レポジトリには、個々のプロジェクト実行環境であるローカルレポジトリと共有の管理場所であるリモートレポジトリがある。ローカルレポジトリでは、ワークツリー内で変更して保存されたファイルをステージングエリアに登録し、それをcommitして、Gitディレクトリに保存するということが行われる。ワークツリーには、untracked、unmodified、modifiedという状態がある。untrackedとは、gitで管理されていないファイルの状態である。unmodifiedとは、gitで管理されているが変更されていないファイルの状態である。modifiedとは、gitで管理されていて変更されたファイルの状態である。リモートレポジトリでは、ローカルレポジトリでcommitされた変更をリモートレポジトリへ反映することやリモートレポジトリにある内容をローカルレポジトリに反映することが行われる。また、gitには、gitフローというものがある。gitフローとは、開発プロジェクトのブランチを効率よく管理するための方法である。ここでいうブランチとは、作業を枝分かれさせることができるものであり、共同作業や並列作業が可能になる。
# Github
Githubとは、リモートレポジトリのホスティングサービスの一つである。これは、オープンソースソフトウェアの主要なポータルサイトである。また、Githubでは、githubフローというシンプルなブランチ戦略を用いた開発フローが用いられる。そして、Githubでのソフトウェア・エンジニアリングでは、clone、fork、template、issues、projectsという機能が用いられる。cloneとは、リモートレポジトリをローカルレポジトリにコピーすることである。forkとは、他ユーザーのリモートレポジトリを自分のリモートレポジトリにコピーすることである。templateとは、既存のレポジトリをテンプレートとして利用し、同じ構成で新しいレポジトリを作成できる機能である。issuesとは、レポジトリに関する問題、課題、バグ、機能追加、質問などを挙げるための機能である。projectsとは、issuesなどを管理するためのテーブルである。
# CI/CD
CIとは、Continuous Integrationの略であり、コード変更を共有リポジトリに頻繁に統合するプロセス自動テストとビルドを定期的に実行し、バグの早期発見と修正を可能にする。CIの自動化により、開発のスムーズな進行を促進する。CDとは、Continuous Deliveryの略であり、コード変更をテスト環境や本番環境に自動的にデプロイするプロセスである。それにより、自動デプロイメントを組み込み、手動操作によるデプロイの必要性を排除することができる。加えて、CDにより、ユーザーフィードバックを迅速に反映可能である。CI/CDパイプラインの基本的なステップは、ソース、ビルド、テスト、デプロイ、検証、モニタリングという順で行われる。ソースとは、コード変更をトリガーにワークフローを起動するというものである。ビルドとは、ソースコードをコンパイルすることである。テストとは、自動テストを実行することである。デプロイとは、テスト済みのコードを本番環境にデプロイすることである。検証とは、デプロイされたアプリケーションの動作確認をすることである。モニタリングとは、本番環境での継続的な監視を行うことである。また、Github Actionsを使用したCI/CDは、ワークフローの定義、トリガー、ジョブ、ステップ、アクション、ランナーという順で行われる。ワークフローの定義は、YAMLファイルで定義し、.github/workflows/ディレクトリに配置する。これは、イベント、ジョブ、ステップで構成されている。トリガーは、プッシュ、プルリクエスト、スケジュールなど様々なイベントでワークフローを起動可能である。ジョブは、並列実行が可能である。ステップは、シェルコマンドの実行やアクションの使用が可能である。アクションは、再利用可能なワークフローの構成要素であり、コミュニティが作成したアクションも利用可能である。ランナーは、Github提供のホステッドランナーまたはセルフホステッドランナーを使用可能である。
# Github pages
CI/CDの実装の例として、Github Pagesがある。Github Pagesとは、Githubにあるリモートレポジトリのな内容をwebpageとして公開することである。HP更改はCI/CDにより自動化することができ、Jekyllを使用することでmarkdownファイルを自動でhtml化することができる。それにより、開発者はmarkdownファイルを作成すれば良くなる。
